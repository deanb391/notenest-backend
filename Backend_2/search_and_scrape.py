...
def main():
    ...
    #The scraping for this will be different since scrapping reddit is a different ball game.
    #Start working on scrapping reddit and cleaning up the text that was scrapped.

    #Won't need this because I've decided to scrape only reddit as extra for now
    """
    for url in secondary_sites:
        scrapes += 1
        if scrapes != 1:
            knowledge_expansion += scraped_content(url)
    """
    ...
    #print(knowledge_expansion)
    #main_expansion = clean_scraped_text(knowledge_expansion)
    #print("AFTER CLEANING\n", main_expansion)
    #print(knowledge_expansion)
    #print("These are youtube videos ", youtube_links)
    #The number of Youtube links will be calculated from implicit personalisation. where 
    #We currently cannot scrape the p tags in reddit. we want to be able to do that.
...
def llama3_8b_8192(user_content):
...
def main():
    user_content = "MILLER INIDICES Mile endres   ystin of notatron  planes witton a cryrral of space lattieThey are Sased e miller index fo hat axis is 2ero (0 have the same index number.The distanc 3 The dsbance d betevein adjacent planes of a set of parallel planes of the indies h, 'l  gren byd d = a x H9== $h{^{}k^{}+^{2$ How to Draw the Plaiu a s 80 Fond the reiprocal o the given miller sridcen These reuproca (. gve the pntercepts made pye miller index fo hat axis is 2ero (0 have the same index number.The distanc 3 The dsbance d betevein adjacent planes of a set of parallel planes of the indies h, 'l  gren byd d = a x H9== $h{^{}k^{}+^{2$ How to Draw the Plaiu a s 80 Fond the reiprocal o the given miller sridcen These reuproca (. gve the pntercepts made py the plane on a, y nd z axes WOLK C+A respectively 2. Draw he cube and select a preper origin and shous i  and Z axes rerectrely. 3. Wih respet  orgm  ma these intercepes and jot though"
    answer = llama3_8b_8192(user_content).split("</think>")[1].strip()
    print(answer)
